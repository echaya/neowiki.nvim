...
wiki.follow_link = function(open_cmd)
  ...
  if filename and filename:len() > 0 then
    ...
    vim.notify(
      string.format('[WikiLink] Input: active_wiki_path="%s", filename="%s"', active_path, filename),
      vim.log.levels.INFO
    )
    ...
    vim.notify(
      string.format('[WikiLink] Joined (unresolved) path: "%s"', joined_path),
      vim.log.levels.INFO
    )
    vim.notify(string.format('[WikiLink] Resolved full_path: "%s"', full_path), vim.log.levels.INFO)
